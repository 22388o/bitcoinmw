...
#[test]
fn simple() {
	let mut a: Box<dyn Simple> = Box::new(ImplA {
		data: "data".into(),
	});
...
#[test]
fn with_params() {
	let mut a: Box<dyn WithParams<Param1, Param2>> = Box::new(ImplA {
		data: "data".into(),
	});
...
#[test]
fn simple_sync() {
	let a: Arc<dyn SimpleSync> = Arc::new(ImplA {
		data: "data".into(),
	});
...
#[test]
fn with_params_sync() {
	let a: Arc<dyn WithParamsSync<Param1, Param2>> = Arc::new(ImplA {
		data: "data".into(),
	});
