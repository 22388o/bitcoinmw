...
//! # The BitcoinMW Logging Crate
//! The BitcoinMW Logging crate handles logging for all crates within BitcoinMW. There is a global static
...
//! straightforward to understand. The logger is largely compatible with the Rust
...
//!     // the log may be initialized globally. We comment this out here to not interfere with
//!     // other tests
//!     /*
//!     log_init!(
//!         AutoRotate(true), // turn on autorotation
//!         LogFilePath("./mylog.log"), // log to our log file
...
//!         LogFilePath(buf.display().to_string().as_str()), // our path
//!         MaxAgeMillis(1000 * 30 * 60), // log max age before rotation
//!         Colors(false), // don't display colors
//!         Backtrace(false) // don't show the backtrace on error/fatal log lines
//!     )?;
//!
//!     logger.init()?;
...
//! LogFilePath may NOT be changed after [`crate::Logger::init`] is called.
//!
//!```
//! use bmw_core::*;
//! use bmw_log::*;
...
//!     )?;
//!     */
//!     info!("begin logging to init logger")?;
//!
//!     set_log_option!(Colors(false))?;
//!
//!     info!("show this without colors!")?;
//!
//!     set_log_option!(Colors(true))?;
//!
